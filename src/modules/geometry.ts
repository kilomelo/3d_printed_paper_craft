...
function packTriangleLayouts(layouts: TriangleUVLayout[]): { packed: PackedTriangleUVLayout[]; side: number } {
  ...
  sorted.forEach((layout) => {
    ...
    packed.forEach((placed) => {
      const points: UVPoint[] = [
        { x: placed.x + placed.width, y: placed.y },
        { x: placed.x, y: placed.y + placed.height },
      ];
      points.forEach((pt) => {
    ...
    for (const candidate of candidates) {
      const probe = { x: candidate.x, y: candidate.y, width: layout.width, height: layout.height };
      let collides = false;
      for (const placed of packed) {
        if (rectsOverlap(probe, { x: placed.x, y: placed.y, width: placed.width, height: placed.height })) {
          collides = true;
          break;
        }
      }
      ...
      const nextMaxX = Math.max(maxX, candidate.x + layout.width);
      const nextMaxY = Math.max(maxY, candidate.y + layout.height);
      const side = Math.max(nextMaxX, nextMaxY);
      const area = nextMaxX * nextMaxY;
      const span = nextMaxX + nextMaxY;

      if (
        side < bestSide - RECT_OVERLAP_EPSILON ||
        (Math.abs(side - bestSide) <= RECT_OVERLAP_EPSILON && area < bestArea - RECT_OVERLAP_EPSILON) ||
        (Math.abs(side - bestSide) <= RECT_OVERLAP_EPSILON && Math.abs(area - bestArea) <= RECT_OVERLAP_EPSILON && span < bestSpan - RECT_OVERLAP_EPSILON) ||
        (Math.abs(side - bestSide) <= RECT_OVERLAP_EPSILON && Math.abs(area - bestArea) <= RECT_OVERLAP_EPSILON && Math.abs(span - bestSpan) <= RECT_OVERLAP_EPSILON && (candidate.y < bestY - RECT_OVERLAP_EPSILON || (Math.abs(candidate.y - bestY) <= RECT_OVERLAP_EPSILON && candidate.x < bestX - RECT_OVERLAP_EPSILON)))
      ) {
        bestX = candidate.x;
        bestY = candidate.y;
        bestSide = side;
    ...
    maxX = Math.max(maxX, bestX + layout.width);
    maxY = Math.max(maxY, bestY + layout.height);
  });
...
function writePackedTriangleUVs(
  ...
  packed.forEach((layout) => {
    ...
    const boxCenterX = layout.x + layout.width * 0.5;
    const boxCenterY = layout.y + layout.height * 0.5;

    layout.localPoints.forEach((point, localIdx) => {
      const x = (point.x - layout.width * 0.5) * paddingScale + boxCenterX;
      const y = (point.y - layout.height * 0.5) * paddingScale + boxCenterY;
...
export function generatePerTriangleUVGeometry(
  ...
  const { packed, side } = packTriangleLayouts(layouts);
