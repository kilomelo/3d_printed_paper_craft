...
type BindGroupPreviewActionsOptions = {
  exportGroupStepBtn: HTMLButtonElement;
  exportGroupStlBtn: HTMLButtonElement;
...
export const bindGroupPreviewActions = (opts: BindGroupPreviewActionsOptions) => {
  ...
  opts.exportGroupStepBtn.addEventListener("click", async () => {
    opts.exportGroupStepBtn.disabled = true;
    try {
      ...
      opts.log(opts.t("log.export.step.fail"), "error");
    } finally {
      opts.exportGroupStepBtn.disabled = false;
    }
  });

  opts.exportGroupStlBtn.addEventListener("click", async () => {
    opts.exportGroupStlBtn.disabled = true;
    try {
      const targetGroupId = opts.getPreviewGroupId();
      if (opts.hasGroupIntersection(targetGroupId)) {
        opts.log(opts.t("log.export.selfIntersect"), "error");
      ...
      opts.exportGroupStlBtn.disabled = false;
    }
  });
